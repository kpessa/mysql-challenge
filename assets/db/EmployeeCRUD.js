...
async function viewAllEmployees(connection) {
  ...
  SELECT e.first_name, e.last_name, job_title as title, d.name, concat("$",FORMAT(salary,0)) as salary, IF(m.first_name is NULL, "", concat(m.first_name, " ", m.last_name)) as manager 
  FROM employees as e
